...
{
    public class ActionPropertiesViewModel : DMViewModelBase
    {
        #region Properties
        ...
        #region Constructor
        public ActionPropertiesViewModel(object token, IActionModel action, ScenarioTypes scenarioType)
        {
            _Token = token;
            _Action = action ?? new LayoutActionModel();
            ...
            else if(_ScenarioType == ScenarioTypes.USER)
            {
                ...
                var resultLinq = resultAPI.Where(p => p.RoutingType == "DM701000");

                foreach (DmComponentMst item in resultLinq)
                {
                    _layout_LayoutList.Add(new SelectionModel()
                    {
                        Key = item.RoutingCode,
                        Value = item.RoutingName,
                        DisplayName = item.RoutingName
                    });
                }
            }
            ...
            _ipcam_compo_CompoList.AddRange(
                    CoPlatformWebClient.Instance.GetIPCamCompoSelectionList(_ScenarioType.ToString()));
        ...
        {
            EventAggregator?.GetEvent<PubSubEvent<ActionPropertiesCloseEvent>>().Publish(
                new ActionPropertiesCloseEvent(Token));
        }
        ...
        private void SaveCommandExecute()
        {
            ...
            EventAggregator?.GetEvent<PubSubEvent<ActionPropertiesSaveEvent>>().Publish(
                new ActionPropertiesSaveEvent(Token, newAction));
        }
